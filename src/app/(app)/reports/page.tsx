...
const fetcher = (url: string) => fetch(url).then(r => r.json() as Promise<OrdersResponse>);
...
export default function ReportsPage() {
  ...
  // put this at module scope (NOT inside a component)
  function useOrders() {
    const { data, error, isLoading } = useSWR<Order>('/api/orders', fetcher);
    return {
      orders: data && 'data' in data ? data.data : [],
      isLoading,
      isError: Boolean(error) || (data && 'error' in data),
    };
  }
  const { orders, isLoading, isError } = useOrders();


  const handleOpenReturnDialog = (items: any[]) => {
    setSelectedOrderItems(items);
    setReturnDialogOpen(true);
  };

  const handleReturnSubmit = async (returns: { productId: string; quantity: number }[]) => {
    try {
      ...
      if (!response.ok) throw new Error('Failed to update returns');
  ...
  const filteredOrders = useMemo(() => {
    ...
    switch (filter) {
      case 'today':
        filtered = orders?.filter((o: any) => format(new Date(o.createdAt), 'yyyy-MM-dd') === format(now, 'yyyy-MM-dd'));
        break;
      case 'yesterday':
        const yesterday = subDays(now, 1);
        filtered = orders?.filter((o: any) => format(new Date(o.createdAt), 'yyyy-MM-dd') === format(yesterday, 'yyyy-MM-dd'));
        break;
      case 'week':
        const startOfThisWeek = startOfWeek(now);
        const endOfThisWeek = endOfWeek(now);
        filtered = orders?.filter((o: any) => isWithinInterval(new Date(o.createdAt), { start: startOfThisWeek, end: endOfThisWeek }));
        break;
      case 'month':
        const startOfThisMonth = startOfMonth(now);
        const endOfThisMonth = endOfMonth(now);
        filtered = orders?.filter((o: any) => isWithinInterval(new Date(o.createdAt), { start: startOfThisMonth, end: endOfThisMonth }));
        break;
      case 'year':
        const startOfThisYear = startOfYear(now);
        const endOfThisYear = endOfYear(now);
        filtered = orders?.filter((o: any) => isWithinInterval(new Date(o.createdAt), { start: startOfThisYear, end: endOfThisYear }));
        break;
      case 'custom':
        if (dateRange?.from && dateRange?.to) {
          filtered = orders?.filter((o: any) => isWithinInterval(new Date(o.createdAt), { start: dateRange.from!, end: dateRange.to! }));
        } else if (dateRange?.from) {
          filtered = orders?.filter((o: any) => format(new Date(o.createdAt), 'yyyy-MM-dd') === format(dateRange.from!, 'yyyy-MM-dd'));
        }
        break;
      default: // 'all'
        filtered = orders;
                ...
                return (
                  <React.Fragment key={order.id}>
                    ...
                    >
                      ...
                      <TableCell>
                        <Badge variant={getPaymentStatus(order).variant}>
                      ...
                      <TableCell>
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            ...
                            <DropdownMenuItem onClick={() => handleOpenReturnDialog(order.items)}>
                                ...
                                {order?.items?.map((product: any, index: number) => {
                                  ...
                                  return (
                                    <TableRow key={index}>
                                      ...
                                      <TableCell>{product?.numberOfDays}</TableCell>
                                      <TableCell className="text-right">₹{product?.rentRate.toFixed(2)}</TableCell>
                                      <TableCell className="text-right">
                                        ₹{(product?.quantity * product?.rentRate * product?.numberOfDays).toFixed(2)}
                                      </TableCell>
                                    </TableRow>
                                  );
                                })}
